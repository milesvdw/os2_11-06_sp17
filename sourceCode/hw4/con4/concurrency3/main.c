	...
	while(true) {
		...
		while(true) {
			...
			if(n_deleters > 0 || n_inserters > 0) {
				//bad time
				if(verbose) printf("\tnot a good time for inserting, sleeping instead\n");
	...
	while(true) {
		...
		while(true) {
			...
			if(n_deleters > 0 || deleters_waiting > 0) {
				//bad time
				if(verbose) printf("\t\tnot a good time for searching, sleeping instead\n");
				pthread_mutex_unlock(&status_lock);
				sleep(1);
			} else {
	...
	while(true) {
		...
		while(true) {
			...
			if(n_deleters > 0 || n_inserters > 0 || n_searchers > 0) {
				deleters_waiting += 1;
				if(verbose) printf("not a good time for deleting, sleeping instead\n");
				pthread_mutex_unlock(&status_lock);
				sleep(1);
			} else {
				n_deleters = 1;
